...
// Fix: Corrected property naming mismatches by using the correct uppercase keys from the types.ts interfaces
export const syncToSupabase = async (data: AppData) => {
  ...
  try {
    ...
    if (data.MATERIALES.length > 0) {
      await supabaseInstance.from('materiales').upsert(
        data.MATERIALES.map(m => ({
          codigo: String(m.CODIGO || '').trim(),
          descripcion: m.DESCRIPCION,
          modelo: m.MODELO,
          marca: m.MARCA,
          precio_un: m.PRECIO_UN || 0,
          en_stock: m.EN_STOCK || 0
        })),
        { onConflict: 'codigo' }
      );
    }
    ...
    if (data.CLIENTES.length > 0) {
      await supabaseInstance.from('clientes').upsert(
        data.CLIENTES.map(c => ({
    ...
    // Listado de clientes válidos (limpios) para evitar errores de FK
    const validClientCodes = new Set(data.CLIENTES.map(c => String(c.COD_CLIENTE || '').trim()));

    // 3. OFs (Filtrar para evitar errores de FK)
    const validOFs = data.ORD_FABRICACIONES.filter(of => {
      const clientCode = String(of.COD_CLIENTE || '').trim();
      return validClientCodes.has(clientCode);
    });

    if (validOFs.length > 0) {
      await supabaseInstance.from('ord_fabricaciones').upsert(
        validOFs.map(of => ({
    ...
    const validOTs = data.ORD_TRABAJOS.filter(ot => {
      ...
      return validOFCodes.has(ofCode);
    });

    if (validOTs.length > 0) {
      await supabaseInstance.from('ord_trabajos').upsert(
        validOTs.map(ot => ({
    ...
    if (data.COTIZACIONES.length > 0) {
      await supabaseInstance.from('cotizaciones').upsert(
