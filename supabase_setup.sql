...
-- Ejecutar esto en el SQL Editor de Supabase para habilitar eliminaciones
...
-- 3. Tabla de Órdenes de Fabricación (Con ON DELETE CASCADE)
CREATE TABLE IF NOT EXISTS ord_fabricaciones (
  of TEXT PRIMARY KEY,
  descripcion_of TEXT,
  cod_cliente TEXT REFERENCES clientes(cod_cliente) ON DELETE CASCADE,
  fecha_entrega TEXT,
  fecha_ocompra TEXT,
  obra_terminada TEXT,
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
...
-- 4. Tabla de Órdenes de Trabajo (Con ON DELETE CASCADE)
CREATE TABLE IF NOT EXISTS ord_trabajos (
  ot TEXT PRIMARY KEY,
  descripcion_ot TEXT,
  ofabricacion TEXT REFERENCES ord_fabricaciones(of) ON DELETE CASCADE,
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
