...
export const CotizacionesManager: React.FC<CotizacionesManagerProps> = ({ data, onDataUpdate }) => {
  ...
  const calculateTotals = (items: CotizacionItem[], imprevistos: number) => {
    ...
    const total = subtotalNeto * (1 + imprevistos / 100);
    ...
    const totalProyecto = subtotalProyecto * (1 + imprevistos / 100);
  ...
  const handleSyncWithDB = () => {
    ...
    // Fix: Corrected typo from IMprevistos to IMPREVISTOS
    const { subtotalNeto, total, totalProyecto } = calculateTotals(updatedItems, editingQuote.IMPREVISTOS || 0);
                    ...
                    <tr key={q.ID} className="hover:bg-slate-50 transition-colors group">
                      <td className="px-8 py-5">
                        <div className="flex items-center gap-2">
                          ...
                          {/* Fix: Wrapped Sparkles in span with title to resolve Lucide prop error */}
