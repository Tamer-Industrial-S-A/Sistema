...
export const ConfiguracionManager: React.FC<ConfiguracionManagerProps> = ({ 
  ...
  const [sbUrl, setSbUrl] = useState(data.CONFIG.supabaseUrl || '');
  const [sbKey, setSbKey] = useState(data.CONFIG.supabaseAnonKey || '');
  ...
  const handleSaveSupabaseCreds = async () => {
    ...
    // Pequeño delay para que initSupabase en App.tsx se ejecute
    setTimeout(async () => {
  ...
  return (
    <div className="space-y-8 pb-20">
      ...
      <div className="bg-white p-8 rounded-[2rem] border border-slate-200 shadow-sm space-y-8">
        <div className="flex justify-between items-start">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="font-black text-xl text-slate-800 uppercase tracking-tight">Credenciales de Supabase</h3>
              <p className="text-sm text-slate-500 font-medium">Configuración de servidor para respaldo en la nube.</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">URL del Proyecto (Project URL)</label>
            <input 
              type="text"
              placeholder="https://xyz.supabase.co"
              className="w-full px-5 py-3 border-2 border-slate-100 rounded-2xl outline-none focus:ring-4 focus:ring-indigo-50 focus:border-indigo-500 font-bold text-slate-700 bg-white"
              value={sbUrl}
              onChange={(e) => setSbUrl(e.target.value)}
            />
          </div>
          <div className="space-y-2">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">API Key Anónima (Anon Key)</label>
            <input 
              type="password"
        ...
        {connectionError && (
          <div className="p-4 bg-red-50 border border-red-100 rounded-xl flex items-center gap-3 text-red-600 text-xs font-bold animate-in shake">
            <AlertCircle size={16} />
            {connectionError}
          </div>
        )}
        ...
        <div className="flex justify-end gap-3">
          <button 
            ...
            className="px-8 py-3 bg-indigo-600 text-white rounded-xl font-black uppercase text-xs tracking-widest shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition active:scale-95 flex items-center gap-2"
          >
            <Save size={18} />
            Probar y Guardar
          </button>
      ...
      <div className="bg-white p-8 rounded-[2rem] border border-slate-200 shadow-sm space-y-6">
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="font-black text-xl text-slate-800 uppercase tracking-tight">Sincronización Cloud</h3>
              <p className="text-sm text-slate-500 font-medium">Respaldo masivo de todas las tablas base.</p>
            </div>
          </div>
          ...
          <div className="flex gap-3">
            <button 
              onClick={onCloudPush}
              className="px-6 py-4 bg-slate-900 text-white rounded-2xl font-black uppercase text-xs tracking-[0.1em] flex items-center gap-3 hover:bg-black transition shadow-xl active:scale-95 disabled:opacity-50 disabled:bg-slate-300"
            ...
            <button 
              onClick={onCloudPull}
              className="px-6 py-4 bg-white text-slate-700 border border-slate-200 rounded-2xl font-black uppercase text-xs tracking-[0.1em] flex items-center gap-3 hover:bg-slate-50 transition active:scale-95 disabled:opacity-50"
              disabled={connectionStatus !== 'CONNECTED'}
            >
        ...
        {connectionStatus !== 'CONNECTED' && (
          <div className="p-4 bg-orange-50 border border-orange-100 rounded-xl flex items-center gap-3 text-orange-600 text-[10px] font-black uppercase">
            <AlertCircle size={16} />
            Establezca una conexión válida arriba para habilitar la transferencia de datos.
          </div>
        )}
      </div>
      ...
      <div className="bg-white p-8 rounded-[2rem] border border-slate-200 shadow-sm space-y-6">
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="font-black text-xl text-slate-800 uppercase tracking-tight">Vincular Carpeta Local (Excel)</h3>
              <p className="text-sm text-slate-500 font-medium">Sincroniza el ERP con archivos físicos en tu PC.</p>
            </div>
          </div>
          ...
          <div className="flex gap-3">
            ...
            >
              <Link2 size={20} />
              {currentDirName ? 'Cambiar Carpeta' : 'Seleccionar Carpeta'}
            </button>
          </div>
        </div>
        ...
        {currentDirName ? (
          <div className="flex items-center gap-4 p-5 bg-green-50 border border-green-100 rounded-[1.5rem]">
            ...
            <div className="flex-1">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Estado: Vinculado localmente</p>
          ...
          <div className="p-5 bg-slate-50 border border-dashed border-slate-200 rounded-[1.5rem] flex items-center gap-3 text-slate-400 italic text-sm">
            ...
            Vincule la carpeta donde se encuentran los archivos MATERIALES, CLIENTES, etc.
          </div>
        )}
      </div>
      ...
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-black text-slate-800 uppercase tracking-tight">Administración de Usuarios</h2>
          <p className="text-sm text-slate-500 font-medium">Accesos y seguridad.</p>
        </div>
        <button 
          ...
          <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-4xl overflow-hidden flex flex-col h-[90vh] animate-in slide-in-from-bottom-8">
            ...
            <div className="p-8 bg-white border-t border-slate-100 flex justify-end gap-4 shadow-2xl">
              ...
              <button 
                ...
                className="px-12 py-5 bg-slate-900 text-white rounded-[1.5rem] font-black uppercase text-sm tracking-widest shadow-xl active:scale-95 transition-all flex items-center gap-3"
              >
                <Save size={20} />
